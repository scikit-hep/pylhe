...
date: 24 September 2026 # APN TODO: update date
...
The library enables memory-efficient streaming of events from XML format (`.lhe`, compressed `.lhe.gz`) and HDF5 format (`.h5`, `.hdf5`, `lheh5`) files through a pythonic iterator interface, allowing researchers to process arbitrarily large event files without loading all events into memory simultaneously.
...
Typically, LHE files are used to describe parton-level events generated by matrix element generators that are passed to parton shower and hadronization programs producing HepMC files [@Dobbs:2001ck;@Buckley:2019xhk;@hans_dembinski_2022_7013498], or directly to analysis frameworks such as Rivet [@Buckley:2010ar;@Bierlich:2019rhm;@Bierlich:2024vqo].
...
Consequently, compression using the `gzip` library is common practice for both formats.

Since `pylhe` version 2.0.0, the library also supports the HDF5-based [@hdf5] LHEH5 format [@Hoche:2019flt;@Bothmann:2023ozs], which allows for significant improvements in write performance while keeping file sizes for event storage under control.
...
`pylhe` completely implements the widely adopted version 3.0, that is the addition of `<scales>`, `<generator>`, and multiple weights via `<initrwgt>`, `<rwgt>`, `<weight>`, `<weights>`, `<wgt>`, and `<weightgroup>` on top of the version 1.0.
Features that were introduced in version 2.0 but removed in version 3.0, such as `<pdfinfo>` or `<clustering>`, are not supported.
...
Unlike the existing C/C++/Fortran interfaces provided by the Monte Carlo event generators,
`pylhe` offers a standalone, pure Python interface that is more accessible and easier to use.
When `pylhe` was first developed, there were no other standalone Python libraries available for reading and writing LHE files.
However, a Python parser is distributed as part of MadGraph [@Alwall:2014hca], while MadAnalysis 5 exposes its C++ LHE reader and writer through a Python interface [@Conte:2012fm].
...
These provide varying degrees of completeness, but none support the LHEH5 format yet.
...
Notably, it has been cited in Higgs studies [@Brehmer:2019gmn;@Stylianou:2023tgg;@Feuerstake:2024uxs], in Supersymmetry (SUSY), Beyond the Standard Model (BSM) and dark matter searches [@Beresford:2018pbt;@Kling:2020iar;@Anisha:2023xmh;@Zhou:2022jgj;@Zhou:2024fjf;@Cheung:2024oxh;@Beresford:2024dsc], and in forward physics studies [@Kling:2022ykt;@Kelly:2021mcd;@Kling:2020mch].
It is also employed in methodological studies involving machine learning techniques for event generation and analysis [@Brehmer:2019xox;@Kofler:2024efb].
