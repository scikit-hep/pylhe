...
As the complexity of Monte Carlo simulations increased, the need for a more flexible and extensible format led to the development of the Les Houches Event (LHE) file format [@Alwall:2006yp].
...
In contrast to LHE files HepMC stores fewer details about the particles, but also contain many more particles per event.
...
The LHE format stores information within `<init>` and multiple `<event>` blocks consisting of whitespace-separated values designed for straightforward parsing in Fortran.
