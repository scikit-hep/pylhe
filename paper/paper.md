...
Since `pylhe` version 2.0.0, the library also supports the HDF5-based LHEH5 format [@hdf5;@Hoche:2019flt;@Bothmann:2023ozs], improving read/write performance and further reducing file sizes for event storage.
...
Features that were introduced in version 2.0 but removed in version 3.0 again, such as `<pdfinfo>` or `<clustering>` are not supported.
...
These provide varying degrees of completeness with none supporting LHEH5 yet.
...
For LHEH5 files the library uses `h5py` [@collette2013pythonhdf5] to read and write the HDF5 format, via the same pythonic data structures making it easy to transparently convert between XML and HDF5 formats.
